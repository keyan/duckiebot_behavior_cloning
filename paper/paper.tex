...
\subsection{Model Architectures} \label{sec:model_arch}
    ...
    The second architecture used a smaller number of total parameters, shared weights across the convolutional layers, and two outputs preceded by small disjoint linear layers (Figure \ref{fig:model_arch}b). We named the initial version of this model the clever name ``modelv0", subsequent experimentation produced a second version ``modelv1" which 
...
\subsection{Training}
All models were trained initially on the aggregated dataset (``dataset A") for 200 epochs using stochastic gradient descent and mean squared error loss. Results of training can be seen in Figure \ref{fig:training_results}.
